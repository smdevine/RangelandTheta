...
precip_summary
tiff(file = file.path(results, 'figures', 'finals', 'precip_summary',  'WY2017_2018_precip.tif'), family = 'Times New Roman', pointsize = 11, width = 3.25, height = 3.25, units = 'in', res=150)
par(mar=c(3, 5, 1, 1))
...
points(WY_summary_GS$mean_T_GS[WY_summary_GS$year %in% 2011:2018], WY_summary_GS$precip_GS_mm[WY_summary_GS$year %in% 2011:2018], cex=1.5)
...
legend('topleft', legend=c('before 1980', 'after 1980'), pch=19, col = c('blue', 'red'), inset = 0.01)
text(8.8, 475, '90th percentile', adj=c(0,0))
text(8.8, 295, 'median precipitation', adj=c(0,0))
text(8.8, 190, '10th percentile', adj=c(0,0))
dev.off()
